...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		...
		//这里可以初始化beanFactory并且读取xml文件，当然如果是springboot这里不会初始化beanfactory,springboot的初始化在springboot逻辑里面进行处理
		refreshBeanFactory();
		// 返回刚刚创建的 BeanFactory
		return getBeanFactory();
	}
	 ...
	 * 首先对 获取的beanFactoryPostProcessors List集合进行遍历看是否实现了BeanDefinitionRegistryPostProcessors接口，
	 * 实现了则调用postProcessBeanDefinitionRegistry方法逻辑，然后把对应的beanFactoryPostProcessor添加到registryProcessors List集合，
	 * 没有实现BeanDefinitionRegistryPostProcessors接口的添加到regularPostProcessors List集合中。
	 */
